...
class altaUsuarioRequest extends FormRequest
{
    public function messages() 
    {
        return [
            ...
            'depen_id.required'   => 'La Unidad Administrativa es obligatoria.'
    ...
    {
        return [  
            'nombre'    => 'min:4|max:80|required', //|regex:/^([0-9a-zA-ZñÑáéíóúÁÉÍÓÚ_-])+((\s*)+([0-9a-zA-ZñÑáéíóúÁÉÍÓÚ_-]*)*)+$/', //regex:/(^([a-zA-z\s]+)?$)/i', 
            'paterno'   => 'min:4|max:80|required', //|regex:/(^([a-zA-z\s]+)?$)/i',
            'materno'   => 'min:4|max:80|required', //|regex:/^([0-9a-zA-ZñÑáéíóúÁÉÍÓÚ_-])+((\s*)+([0-9a-zA-ZñÑáéíóúÁÉÍÓÚ_-]*)*)+$/',
